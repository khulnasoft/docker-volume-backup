...
docker build -t khulnasoft/docker-volume-backup:test-sandbox .

if [ ! -z "$BUILD_IMAGE" ]; then
  docker build -t khulnasoft/docker-volume-backup:$IMAGE_TAG $(dirname $(pwd))
fi

docker save khulnasoft/docker-volume-backup:$IMAGE_TAG -o $tarball
...
for dir in $(find $find_args | sort); do
  ...
  docker run $docker_run_args khulnasoft/docker-volume-backup:test-sandbox
